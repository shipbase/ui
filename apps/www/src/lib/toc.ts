...
function injectChild(items: TocItem[], item: TocItem, indexPath: number[]): void {
  const lastItem = items.at(-1)
  if (!lastItem || lastItem.depth >= item.depth) {
    items.push(item)
  } else {
    injectChild(lastItem.children, { ...item, id: `${indexPath.join(".")}` }, [...indexPath])
    return
  }
}
...
export function generateToCTree(headings: MarkdownHeading[]) {
  ...
  const toItem = (current: MarkdownHeading, parent?: MarkdownHeading): TocItem => ({
