...
export async function resolvePackageJson(options: { cwd?: string } = {}) {
  return safeResolveAndRead(path.join(options?.cwd || process.cwd(), "package.json"))
}

export async function resolveTSConfig(options: { cwd?: string } = {}) {
  const { cwd = process.cwd() } = options

  const result = await safeResolvePath(path.join(withTrailingSlash(cwd), "tsconfig.json"))
