...
export async function init(initialLibrary: Config["library"]) {
  ...
  try {
    await prepare()
    userConfig = await promptConfig({ library: initialLibrary })
...
async function promptConfig(defaultConfig: Partial<UserConfig>) {
  console.log(
    defaultConfig,
    LIBRARIES,
    LIBRARIES.findIndex((lib) => lib.value === defaultConfig.library)
  )
  const result = await prompts(
    [
      {
        type: defaultConfig.library ? undefined : "select",
        name: "library",
